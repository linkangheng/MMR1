...
# 目录列表
root_dirs = ["/data/dataspace/jihe_train_data/sub1"] # "dataset/jihe_train_data/sub2"
...
def save_json(output_dir, file_name, data_list):
    output_path = os.path.join(output_dir, file_name)
...
for rootdir, filename in tqdm(json_files):
    ...
    # **存储数据**
    data_list.append({"image": img_path, "problem": problem, "solution": solution})
    data_list2.append({"image": img_path, "problem": problem, "solution": solution2})
...
# **存储剩余数据**
if data_list:
    file_name = f"train.json"
    save_json(output_dir, file_name, data_list)
    save_json(output_dir2, file_name, data_list2)