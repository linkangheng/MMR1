...
OUTPUT_DIR="/mnt/jfs-test/yjs/results/perpo/llava1.5-7b/grpo_rule-${reward_rule[$machine_id]}-${reward_scale[$machine_id]}scale-baseline${reward_baseline[$machine_id]}_${num_generations[$machine_id]}roll_${learning_rate[$machine_id]}lr_${kl_approximator[$machine_id]}kl_${order_dataset[$machine_id]}order_${entropy_reg[$machine_id]}enpy_${entropy_weight[$machine_id]}w_${temperature_func[$machine_id]}t_${sync_ref_model[$machine_id]}sync_${ref_model_mixup_alpha[$machine_id]}alpha_${ref_model_sync_steps[$machine_id]}steps_groundingprompt"
